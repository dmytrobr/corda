...
class WithReferencedStatesFlow<T : Any>(
    ...
    companion object {
        ...
        object FAILURE : ProgressTracker.Step("Reference states are out of date! Waiting for updated states...")
        object SUCCESS : ProgressTracker.Step("Flow was ran successfully.")

        @JvmStatic
        fun tracker() = ProgressTracker(ATTEMPT, FAILURE, SUCCESS)
    }
        ...
        while (true) {
            ...
            when (processedResult) {
                is FlowResult.Success<*> -> {
                    ...
                    return processedResult.value as T
                }
                is FlowResult.Conflict -> {
                    val conflicts = processedResult.stateRefs
                    logger.debug("Flow ${flowLogic.javaClass.name} failed due to reference state conflicts: " +
                            "$conflicts. Retrying flow...")
                    // Only set to FAILURE once.
                    if (progressTracker.currentStep != FAILURE) {
                        progressTracker.currentStep = FAILURE
                    }
